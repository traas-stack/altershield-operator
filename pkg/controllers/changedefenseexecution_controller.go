...
func (r *ChangeDefenseExecutionReconciler) resumeProgress(in client.Object) error {
	if in.GetAnnotations() == nil {
		in.SetAnnotations(make(map[string]string))
	}
	in.GetAnnotations()[constants.AnnotationProgressTimestamp] = strconv.FormatInt(time.Now().UnixMilli(), 10)
...
func (r *ChangeDefenseExecutionReconciler) processObserving(changeDefenseExec *v1alpha1.ChangeDefenseExecution,
	...
	if expectedTime.After(time.Now()) {
		klog.Infof("continue observing for ChangeDefenseExecution %v until %v, current %v",
			klog.KObj(changeDefenseExec), expectedTime.String(), currentTime.String())
