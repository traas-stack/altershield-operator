...
func validate(ctx context.Context, req *admission.Request, handler *Handler, allowLatest bool) admission.Response {
	...
	if shouldBlock(latestDefenseExec) {
		return webhookutils.AdmissionDeniedWithLog(fmt.Sprintf("current defense execution %v phase is %v",
			latestDefenseExecModel.BuildChangeDefenseExecutionName(), latestDefenseExec.Status.DefenseStatus.Phase))
	}
	return admission.Allowed("validation pass")
}

func shouldBlock(cde *v1alpha1.ChangeDefenseExecution) bool {
	if v1alpha1.DefensePhaseProgressing == cde.Status.DefenseStatus.Phase {
		return false
	}
	return !(len(cde.Spec.DefenseStrategy.Workload.Steps) == cde.Status.DefenseStatus.CurrentBatch &&
		cde.Status.DefenseStatus.Phase == v1alpha1.DefensePhasePassed)
}
