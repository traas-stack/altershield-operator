...
func WorkloadUpdateMutation(ctx context.Context, req *admission.Request, handler *Handler) admission.Response {
	...
	klog.Infof("found targeted ChangeDefense: %v", klog.KObj(changeDefense))
	...
	klog.Infof("submit altershield change defense exec result: %v", defenseExec)

	nodeID, err := defensePreCheck(handler, defenseExec)
	if err != nil {
		return admission.Errored(http.StatusBadRequest, err)
	}
	klog.Infof("submit change defense pre-check result: nodeID=%v", nodeID)
